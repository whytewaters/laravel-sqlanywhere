...
class SQLAnywherePrepared {
    ...
    protected $__dbinfo;
    protected $__boundParams = [];
    protected $__result;

    public function __construct($sql_string, $connection, array $dbinfo) {
     ...
     */
    public function bindParam($mixed, &$variable, $type = null, $length = null) {
        if (is_string($mixed)) {
    ...
    public function bindValue($mixed, $variable, $type = null, $length = null) {
        if (is_string($mixed)) {
            $this->__boundParams[$mixed] = $variable;
        } else {
            $this->__boundParams[] = $variable;
        ...
        if (count($array) > 0) {
            foreach ($array as $k => $v) {
                if (!is_int($k) && substr($k, 0, 1) === ':') {
                    ...
                    $tempf[] = $k;
                    $tempr[] = "'" . sasql_escape_string($connection, $v) . "'";
                } else if (!is_int($k)) {
                    if (!isset($tempf)) {
                        $tempf = $tempr = [];
                    }
    ...
    public function __setErrors($er) {
        ...
        $this->__errorCode = &$er;
        $this->__errorInfo = [$this->__errorCode, $errno, $errst];
        $this->__result = false;
